...
const HouseReport = () => {
    ...
    const groupByHouse = () => {
        return requests.reduce((acc, request) => {
            const houseNum = houseMap[request.houseNo];
            if (!acc[houseNum]) {
                acc[houseNum] = [];
            }
            acc[houseNum].push(request);
            return acc;
        }, {});
    };
    ...
    const groupedRequests = groupByHouse();

    return (
        <>
            <Navbar />
            <div>
                <h1>House Wise Details</h1>
                {Object.keys(groupedRequests).map(houseNum => (
                    <div key={houseNum}>
                        <h2>House No: {houseNum}</h2>
                        <table>
                            <thead>
                                <tr>
                                    <th>ID</th>
                            ...
                            <tbody>
                                {groupedRequests[houseNum].map(request => (
                                    <tr key={request._id}>
                                        <td>{request._id}</td>
